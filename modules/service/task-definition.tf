...
# resource "aws_ecs_task_definition" "task-prod" {
#   family = format("%s-%s", var.cluster_name-prod, var.service_name)

#   container_definitions    = data.template_file.task-prod.rendered
#   requires_compatibilities = ["FARGATE"]
#   network_mode             = "awsvpc"
#   cpu                      = var.desired_task_cpu
#   memory                   = var.desired_task_mem
